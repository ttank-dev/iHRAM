...
export async function middleware(request: NextRequest) {
  ...
  if (
    ...
    request.nextUrl.pathname === '/merchant/signup'
  ) {
    console.log('✅ Allowing login page:', request.nextUrl.pathname)
    return NextResponse.next()
  }
  ...
  try {
    ...
    if (request.nextUrl.pathname.startsWith('/merchant/dashboard')) {
      console.log('🔒 Protecting /merchant/dashboard')
    ...
    // Redirect logged-in merchants away from login
    if (request.nextUrl.pathname === '/merchant/login' && user) {
      const { data: agency } = await supabase
        .from('agencies')
        .select('is_active')
        .eq('user_id', user.id)
...
export const config = {
  matcher: [
    ...
    // Match /admin/* but EXCLUDE /admin/login
    '/admin/((?!login$).*)',
  ],
}