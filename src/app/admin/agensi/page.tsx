...
/* ── ActionButtons: TOP-LEVEL — 2×2 grid, same as packages page ── */
...
export default function AdminAgensiPage() {
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: `
        .aa,.aa *,.aa *::before,.aa *::after{box-sizing:border-box}
        .aa{width:100%;max-width:1200px}

        /* Header */
        .aa-title{font-size:28px;font-weight:700;color:#2C2C2C;margin:0 0 6px}
        .aa-subtitle{font-size:15px;color:#888;margin:0 0 24px}

        /* Stats */
        .aa-stats-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:20px}
        .aa-stat-card{background:white;border-radius:12px;padding:18px;border:2px solid #E5E5E0;cursor:pointer;transition:all .2s}
        .aa-stat-card:hover{transform:translateY(-2px);box-shadow:0 4px 12px rgba(0,0,0,.06)}
        ...
        .aa-table-header{display:grid;grid-template-columns:2fr 1.2fr 0.6fr 0.6fr 0.8fr 210px;padding:12px 18px;background:#F5F5F0;border-bottom:1px solid #E5E5E0;font-size:11px;font-weight:700;color:#888;text-transform:uppercase;letter-spacing:.5px}
        .aa-table-row{display:grid;grid-template-columns:2fr 1.2fr 0.6fr 0.6fr 0.8fr 210px;padding:14px 18px;border-bottom:1px solid #f0f0ec;align-items:center;transition:background .1s}
        ...
        .aa-cell-act{display:flex;align-items:center}
        ...
        .aa-badge.verified{background:rgba(16,185,129,.1);color:#10B981}
        .aa-badge.pending {background:rgba(245,158,11,.1); color:#F59E0B}
        .aa-badge.inactive{background:rgba(239,68,68,.1);  color:#EF4444}

        /* ── ACTION BUTTONS — exact same as packages page ── */
        .aa-actions{display:grid;grid-template-columns:1fr 1fr;gap:4px;width:210px}
        .aa-btn{
          height:30px;
          padding:0 8px;
          border:none;
        ...
        .aa-btn-slate {background:#E2E8F0;color:#334155}
        ...
        .aa-card{background:white;border-radius:12px;border:1px solid #E5E5E0;padding:14px;margin-bottom:8px}
        ...
        .aa-card-stats{display:flex;gap:16px;align-items:center;margin-bottom:10px;font-size:13px;color:#666}
        .aa-card-date{margin-left:auto;font-size:12px;color:#aaa}
        /* Mobile card actions — same as packages page */
        .aa-card-acts{display:grid;grid-template-columns:1fr 1fr;gap:4px;padding-top:10px;border-top:1px solid #f0f0ec}
        .aa-card-acts .aa-actions{width:100%;display:contents}
        .aa-card-acts .aa-btn{height:34px;font-size:12px}
        ...
        /* Pagination */
        .aa-pagination{display:flex;align-items:center;justify-content:center;gap:8px;padding:16px;flex-wrap:wrap;border-top:1px solid #f0f0ec}
        .aa-pg-btn{padding:8px 16px;background:white;border:1px solid #E5E5E0;border-radius:8px;font-size:13px;font-weight:600;color:#555;cursor:pointer;transition:all .15s;white-space:nowrap}
        ...
        .aa-pg-num{width:36px;height:36px;border:1px solid #E5E5E0;border-radius:8px;background:white;font-size:13px;font-weight:600;color:#555;cursor:pointer;transition:all .15s;display:flex;align-items:center;justify-content:center}
        ...
        @media(min-width:640px) and (max-width:1023px){
          .aa-table-header{grid-template-columns:2fr 0.6fr 0.6fr 0.8fr 210px}
          .aa-table-row   {grid-template-columns:2fr 0.6fr 0.6fr 0.8fr 210px}
        ...
        @media(max-width:639px){
          ...
          .aa-stat-card{padding:10px 8px;border-radius:8px}
          .aa-stat-icon{width:32px;height:32px;font-size:15px;margin-bottom:8px}
          .aa-stat-value{font-size:18px}
          .aa-title{font-size:20px}
          .aa-subtitle{font-size:12px;margin-bottom:16px}
          .aa-search-bar{padding:8px 10px;border-radius:8px}
          .aa-search-input{font-size:13px}
      ...
      <div className="aa">
        ...
        <div className="aa-cards-wrap">
            ...
            return (
              <div key={agency.id} className="aa-card">
                ...
                <div className="aa-card-stats">
                  <div><span className="aa-count">{getPackageCount(agency)}</span> packages</div>
                  <div><span className="aa-count">{getReviewCount(agency)}</span> reviews</div>
                  <div className="aa-card-date">
                    {new Date(agency.created_at).toLocaleDateString('en-MY', { year: 'numeric', month: 'short' })}
                  </div>
                </div>
                <div className="aa-card-acts">
                  <ActionButtons
                    agency={agency}
                    onToggleVerify={handleToggleVerify}
                    onRevokeVerify={handleRevokeVerify}
          ...
          {totalPages > 1 && (
            <div className="aa-pagination" style={{ borderTop: 'none' }}>
              <button className="aa-pg-btn" onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1}>← Prev</button>
              <div className="aa-pg-pages">
                {Array.from({ length: totalPages }, (_, i) => i + 1)
                  .filter(p => p === 1 || p === totalPages || Math.abs(p - currentPage) <= 1)
