  ...
  return (
    <div>
      <div style={{ marginBottom: '32px' }}>
        ...
        <p style={{ fontSize: '16px', color: '#666' }}>Moderate agency news feed posts</p>
      </div>
      ...
      <div style={{ backgroundColor: 'white', borderRadius: '16px', padding: '24px', border: '1px solid #E5E5E0', marginBottom: '24px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: '16px' }}>
          <div>
            ...
            <input type="text" placeholder="Search content or agency..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
      ...
      <div style={{ backgroundColor: 'white', borderRadius: '16px', border: '1px solid #E5E5E0', overflow: 'hidden' }}>
        <div style={{ padding: '24px', borderBottom: '1px solid #E5E5E0' }}>
          <h2 style={{ fontSize: '20px', fontWeight: 'bold', color: '#2C2C2C' }}>News Feed Posts</h2>
        </div>
        ...
        {filteredPosts.length === 0 ? (
          ...
          <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {filteredPosts.map((post: any) => {
              ...
              return (
                <div key={post.id} style={{ padding: '20px', backgroundColor: '#F5F5F0', borderRadius: '12px', display: 'flex', gap: '16px' }}>
                  {firstImage && (
                    <div style={{
                      width: '160px', height: '160px', borderRadius: '8px',
                      backgroundImage: `url(${firstImage})`, backgroundSize: 'cover', backgroundPosition: 'center',
                      flexShrink: 0, border: '1px solid #E5E5E0'
                    }} />
                  )}
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '8px' }}>
                      ...
                      {/* ✅ Updated badge: Published / Unpublished */}
                      <span style={{
                    ...
                    {post.title && (
                      <h3 style={{ fontSize: '16px', fontWeight: '600', color: '#2C2C2C', marginBottom: '8px' }}>{post.title}</h3>
                    )}

                    <p style={{ fontSize: '14px', color: '#666', lineHeight: '1.6', marginBottom: '8px' }}>{post.content}</p>

                    {post.images && post.images.length > 0 && (
                      <div style={{ display: 'inline-block', padding: '4px 8px', backgroundColor: '#E3F2FD', borderRadius: '4px', fontSize: '12px', color: '#2196F3', marginBottom: '12px' }}>
