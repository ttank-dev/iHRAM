...
export default function PakejSayaPage() {
  ...
  const handleDelete = async (id: string, title: string) => {
    ...
    if (agencyId) await loadPackages(agencyId)
    setDeleting(null)
  }
  ...
  const handleDuplicate = async (pkg: any) => {
    ...
    if (error) { alert('Gagal duplicate: ' + error.message); return }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        <div>
          <h1 style={{ fontSize: '32px', fontWeight: 'bold', color: '#2C2C2C', marginBottom: '8px' }}>Pakej Saya</h1>
          <p style={{ fontSize: '15px', color: '#666' }}>Urus semua pakej umrah anda</p>
        </div>
        <Link href="/merchant/dashboard/pakej/new" style={{
          ...
          <span>➕</span><span>Tambah Pakej</span>
        </Link>
      </div>
      ...
      {packages.length > 0 ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {packages.map((pkg) => (
            <div key={pkg.id} style={{ backgroundColor: 'white', borderRadius: '12px', padding: '24px', border: '1px solid #E5E5E0' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                <div style={{ flex: 1 }}>
                  <h3 style={{ fontSize: '20px', fontWeight: '700', color: '#2C2C2C', marginBottom: '8px' }}>{pkg.title}</h3>
              ...
              <div style={{ display: 'flex', gap: '8px', marginTop: '16px', paddingTop: '16px', borderTop: '1px solid #E5E5E0' }}>
                <Link href={`/merchant/dashboard/pakej/edit/${pkg.id}`} style={{
                  padding: '8px 16px', backgroundColor: '#B8936D', color: 'white',
                  borderRadius: '8px', fontSize: '14px', fontWeight: '600', textDecoration: 'none'
                }}>✏️ Edit</Link>
                <button onClick={() => handleDuplicate(pkg)} style={{
                  padding: '8px 16px', backgroundColor: '#F5F5F0', color: '#2C2C2C',
                  border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: '600', cursor: 'pointer'
                }}>📋 Duplicate</button>
                <Link href={`/pakej/${pkg.slug}`} target="_blank" style={{
                  padding: '8px 16px', backgroundColor: '#F5F5F0', color: '#2C2C2C',
                  borderRadius: '8px', fontSize: '14px', fontWeight: '600', textDecoration: 'none'
                }}>👁️ Preview</Link>
                <button onClick={() => handleDelete(pkg.id, pkg.title)} disabled={deleting === pkg.id} style={{
                  padding: '8px 16px', backgroundColor: deleting === pkg.id ? '#CCC' : '#FEE',
                  color: deleting === pkg.id ? '#666' : '#C33', border: 'none', borderRadius: '8px',
                  fontSize: '14px', fontWeight: '600', cursor: deleting === pkg.id ? 'not-allowed' : 'pointer', marginLeft: 'auto'
                }}>
        ...
        <div style={{ backgroundColor: 'white', borderRadius: '16px', padding: '80px 40px', textAlign: 'center', border: '1px solid #E5E5E0' }}>
          ...
          <h3 style={{ fontSize: '24px', fontWeight: 'bold', color: '#2C2C2C', marginBottom: '12px' }}>Tiada Pakej Lagi</h3>
          <p style={{ fontSize: '16px', color: '#666', marginBottom: '24px' }}>Mulakan dengan menambah pakej umrah pertama anda</p>
          <Link href="/merchant/dashboard/pakej/new" style={{
            display: 'inline-block', padding: '14px 32px', backgroundColor: '#B8936D',
            color: 'white', borderRadius: '10px', fontSize: '16px', fontWeight: '700', textDecoration: 'none'
          }}>Tambah Pakej</Link>
        </div>
      )}
    </div>
